...
const FileUploader = ({ setPayload, roll, name }) => {
  const fileUploader = useRef();

  const handleFileChange = (e) => {
    setPayload((prev) => ({
      ...prev,
      [roll]: { file: e.target.files[0], name: e.target.files[0].name, name },
    }));
  };
...
export default function Config() {
  ...
  const fileSubmit = () => {
    ...
    Object.entries(selectedFiles).forEach(([roll, fileObj]) => {
      ...
      formData.append("names", fileObj.name);
    });

    axios
      .post("http://127.0.0.1:8000/cours", formData)
      .then((response) => {})
      .catch((error) => {
        console.error("Error:", error);
      });
                ...
                <TableCell>
                  {selectedFiles[item[1]] && (
                    <StagingArea
                      data={[
                        {
                          name: selectedFiles[item[1]].name,
                        },
                      ]}
                    />
                  )}
